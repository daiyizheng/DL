...
mnist = datasets.MNIST(
    root='./AIGC/datasets/', train=True, download=True, transform=transforms.Compose(
...
for epoch in range(opt.n_epochs):                               ## epoch:50
    for i, (imgs, _) in enumerate(dataloader):                  ## imgs:(64, 1, 28, 28)     _:label(64)
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(fake_img.data[:25], "./AIGC/results/gan/images/%d.png" % batches_done, nrow=5, normalize=True)

## 保存模型
torch.save(generator.state_dict(), './AIGC/results/gan/generator.pth')
torch.save(discriminator.state_dict(), './AIGC/results/gan/discriminator.pth')
