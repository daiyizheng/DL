...
class WGCNA(GeneExp):
    ...
    def __init__(self, name='WGCNA',
        ...
        self.TPMcutoff = TPMcutoff # 表达量低于TPMcutoff
        self.cut = cut
        ...
        self.RsquaredCut = RsquaredCut # R平方裁剪无标度网络的power；介于0到1之间（默认值：0.9）
        self.MeanCut = MeanCut # 选择无标度网络power的平均连接性（默认值：100）
        self.powers = powers
        self.power = 0
        self.sft = None
    ...
    def preprocess(self, show=True):
        ...
        # Check that all genes and samples have sufficiently low numbers of missing values. 检查所有基因和样本的缺失值数量是否足够低
    ...
    def findModules(self, kwargs_function={'cutreeHybrid': {'deepSplit': 2, 'pamRespectsDendro': False}}):
        ...
        # Turn adjacency into topological overlap 将邻接转化为拓扑重叠
        kwargs = dict()
        ...
        # Call the hierarchical clustering function 调用层次聚类函数
        self.geneTree = linkage(a, method="average")

        # Module identification using dynamic tree cut 使用动态树切割进行模块识别
        kwargs = dict()
        ...
        # Convert numeric labels into colors 将数字标签转换为颜色
        kwargs = dict()
        ...
        # Calculate dissimilarity of module eigengenes 计算模块特征基因的相异性
        ...
        if MEDiss.shape != (1, 1):
            ...
            # The merged module colors; Rename to moduleColors 合并后的模块颜色；重命名为moduleColors
            self.datExpr.var['moduleColors'] = merge['colors']
        else:
            self.datExpr.var['moduleColors'] = ["black"] * self.datExpr.shape[1]

        # Construct numerical labels corresponding to the colors 构造与颜色对应的数字标签
        ...
        # Eigengenes of the new merged modules: 新合并模块的特征基因：
        self.MEs = merge['newMEs']

        # Recalculate MEs with color labels 重新计算带有颜色标签的ME
        kwargs = dict()
        if 'moduleEigengenes' in list(kwargs_function.keys()):
            kwargs = kwargs_function['moduleEigengenes']
    ...
    # Call the network topology analysis function 网络拓扑分析功能
    @staticmethod
    def pickSoftThreshold(data, dataIsExpr=True, weights=None, RsquaredCut=0.9, MeanCut=100, powerVector=None,
        ...
        while startG < nGenes:
            ...
            if dataIsExpr:
                ...
                corx = np.corrcoef(corOptions.x[0], corOptions.y[0], rowvar=False) # NumPy 库函数 np.corrcoef() 仅关注并计算 Pearson 相关值。
            ...
            for j in range(nPowers):
                ...
                datk_local[:, j] = np.nansum(corxCur, axis=0) - 1 # 求和忽略NAN
                corxPrev = corxCur
    ...
    def TomSimilarityFromAdj(adjMat, TOMDenom, TOMType):
        ...
        if TOMType == 0:  # unsigned
            ...
            tom = np.fabs((L + adjMat)) / (MINK + 1 - np.fabs(adjMat)) # 绝对值
        np.fill_diagonal(tom, 1) ## 填充任意维数的给定数组的主对角线。
        return tom
    ...
    def orderMEs(MEs, greyLast=True, greyName="MEgrey", orderBy=0, order=None, useSets=None):
        ...
        重新排序给定的（特征）向量，使相似的向量（通过相关性测量）彼此相邻。
